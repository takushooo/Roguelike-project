...
public class Game {
    ...
    private Monster createRandomMonster() {
        List<MonsterWeight> monsterWeights = new ArrayList<>();
        monsterWeights.add(new MonsterWeight(new Goblin(), 100));
        monsterWeights.add(new MonsterWeight(new Orc(), 50));
        monsterWeights.add(new MonsterWeight(new Dragon(), 5));

        int totalWeight = monsterWeights.stream().mapToInt(MonsterWeight::getWeight).sum();
        int randomValue = random.nextInt(totalWeight);

        int currentWeight = 0;
        for (MonsterWeight mw : monsterWeights) {
            currentWeight += mw.getWeight();
            if (randomValue < currentWeight) {
                return mw.getMonster();
            }
        }
